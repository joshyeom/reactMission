...
import { isDarkThemeState } from "./recoil/recoilState.tsx";
import DrawerSide from "./DrawerSide.tsx";
import { useState } from 'react';
import { Link } from 'react-router-dom'; 
import { cartItemCountState } from "./recoil/recoilState.tsx";

export default function Nav({handleThemeToggle}) {
  ...
  const handleClick = () => {
    ...
    handleThemeToggle(newIsDarkTheme);
  };
  ...
  return(
    ...
    <div className="flex items-center px-2">
      <label className="swap swap-rotate mr-2 sm:mr-4">
        <input type="checkbox" className="js-theme" checked={isDarkTheme}
  ...
  <DrawerSide isOpen={!isOpen} handleToggle={handleToggle} />
  </>
  );
}